...
main( int argc, char *argv[ ] )
{
	...
	//fprintf( stderr, "OpenMP is supported -- version = %d\n", _OPENMP );
		...
		// note note: if you are windows, take the ", stderr" out of this list!
			...
			float da = Length( holeax - PinAx, holeay - PinAy );
            if( da - holear + PinAr <= 0 )
            {
                float db = Length( holebx - PinBx, holeby - PinBy );
                if( db - holebr + PinBr <= 0 )
                {
                    float dc = Length( holecx - PinCx, holecy - PinCy );
                    if( dc - holecr + PinCr <= 0 )
                        numSuccesses++;
                }
            }
